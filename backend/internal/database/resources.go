...
var resourceQueries = map[string]resourceQuery{
	...
	"agents":          {"agents", "id,organization_id,name,hostname,os,arch,version,status,last_seen_at,registered_at,capabilities,labels,network_zone,identity_fingerprint,identity_rotated_at", "organization_id=$1", "registered_at DESC"},
	"jobs":            {"analysis_jobs", "id,organization_id,module_id,asset_id,scope_id,agent_id,requested_by,parameters,risk_class,status,created_at,queued_at,started_at,completed_at,timeout_at,rejection_code", "organization_id=$1", "created_at DESC"},
	"schedules":       {"schedules", "id,organization_id,module_id,scope_id,agent_id,frequency_seconds,enabled,next_run_at,created_by,created_at", "organization_id=$1", "created_at DESC"},
	...
	"evidence":        {"evidence", "id,organization_id,job_id,observation_id,finding_id,source,content_type,summary,structured_data,checksum,raw_access_required,created_at,(storage_key IS NOT NULL) AS has_raw_artifact", "organization_id=$1", "created_at DESC"},
