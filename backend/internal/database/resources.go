...
var resourceQueries = map[string]resourceQuery{
	...
	"agents":            {"agents", "id,organization_id,name,hostname,os,arch,version,protocol_version,contract_version,capability_schema_version,compatibility_status,status,last_seen_at,registered_at,capabilities,capabilities_manifest,capabilities_hash,available_slots,running_jobs,health_summary,labels,network_zone,identity_fingerprint,identity_rotated_at,certificate_serial,certificate_expires_at,certificate_rotation_status,signing_key_id", "organization_id=$1", "registered_at DESC"},
