...
var resourceQueries = map[string]resourceQuery{
	...
	"agents":            {"agents", "id,organization_id,name,hostname,os,arch,version,protocol_version,contract_version,capability_schema_version,compatibility_status,status,last_seen_at,registered_at,capabilities,capabilities_manifest,capabilities_hash,available_slots,running_jobs,health_summary,labels,network_zone,identity_fingerprint,identity_rotated_at,certificate_serial,certificate_not_before,certificate_expires_at,certificate_rotation_status,previous_certificate_fingerprint,last_certificate_rotation_at,signing_key_id", "organization_id=$1", "registered_at DESC"},
	...
	"jobs":              {"analysis_jobs", "id,organization_id,module_id,asset_id,service_id,diagnostic_run_id,vantage_point_id,scope_id,agent_id,requested_by,parameters,risk_class,protocol_version,authorization_reference,status,created_at,queued_at,started_at,completed_at,timeout_at,rejection_code,result_identity,result_version,signing_key_id,signature_algorithm,(signature IS NOT NULL) AS signed,CASE WHEN signature IS NULL THEN 'NOT_SIGNED' WHEN status IN ('RUNNING','SUCCEEDED','FAILED','CANCELLED','TIMED_OUT') THEN 'AGENT_ACCEPTED' ELSE 'PENDING' END AS signature_validation_status,signature_issued_at", "organization_id=$1", "created_at DESC"},
	"artifacts":         {"artifacts", "id,organization_id,job_id,type,direction,content_type,original_name,size_bytes,sha256,status,created_at,expires_at,uploaded_by_agent_id,verified_at", "organization_id=$1", "created_at DESC"},
	...
	"observations":      {"observations", "id,organization_id,asset_id,module_id,job_id,evidence_id,category,status,severity,confidence,title,summary,meaning,impact,suggested_action,observed_at,evidence_count", "organization_id=$1", "observed_at DESC"},
	...
	"evidence":          {"evidence", "id,organization_id,job_id,observation_id,artifact_id,finding_id,module_id,agent_id,vantage_point_id,source,artifact_kind,content_type,summary,structured_data,checksum,size_bytes,observed_at,raw_access_required,created_at,(storage_key IS NOT NULL) AS has_raw_artifact", "organization_id=$1", "created_at DESC"},
