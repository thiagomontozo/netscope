...
func (h AgentArtifacts) Create(w http.ResponseWriter, r *http.Request) {
	...
	if manifest.ProtocolVersion != "1.0" || manifest.OrganizationID != org || manifest.JobID == nil || manifest.Direction != artifacts.DirectionFromAgent || !allowedType[manifest.Type] || !allowedArtifactContentType(manifest.ContentType) || manifest.SizeBytes < 0 || manifest.SizeBytes > h.MaxUploadBytes || checksumErr != nil || len(checksumBytes) != 32 {
		middleware.WriteError(w, r, http.StatusBadRequest, "ARTIFACT_MANIFEST_INVALID", "artifact manifest violates transfer policy")
		return
	}
	storageKey := artifactStorageKey(org, *manifest.JobID, manifest.ArtifactID)
...
func (h AgentArtifacts) Authorize(w http.ResponseWriter, r *http.Request) {
	...
	var direction, status, key string
	var size int64
	err := h.Pool.QueryRow(r.Context(), `SELECT a.direction,a.status,a.size_bytes,a.storage_key FROM artifacts a JOIN analysis_jobs j ON j.id=a.job_id AND j.organization_id=a.organization_id WHERE a.organization_id=$1 AND a.id=$2 AND a.job_id=$3 AND j.agent_id=$4 AND j.status<>'CANCELLED'`, org, artifactID, input.JobID, agent).Scan(&direction, &status, &size, &key)
	if err != nil || status == "EXPIRED" || (input.Purpose == artifacts.PurposeDownload && (direction != artifacts.DirectionToAgent || status != "AVAILABLE")) || (input.Purpose == artifacts.PurposeUpload && (direction != artifacts.DirectionFromAgent || status != "PENDING" && status != "FAILED")) {
	...
	if (input.Purpose == artifacts.PurposeDownload && size > h.MaxDownloadBytes) || (input.Purpose == artifacts.PurposeUpload && size > h.MaxUploadBytes) {
		_, _ = h.Pool.Exec(r.Context(), `INSERT INTO audit_events(organization_id,actor_agent_id,event_type,resource_type,resource_id,outcome) VALUES($1,$2,'artifact.size_rejected','artifact',$3,'failure')`, org, agent, artifactID)
		middleware.WriteError(w, r, http.StatusRequestEntityTooLarge, "ARTIFACT_SIZE_LIMIT_EXCEEDED", "artifact exceeds transfer policy")
		return
	}
	if input.Purpose == artifacts.PurposeUpload && status == "FAILED" {
		if exists, existsErr := h.Storage.Exists(r.Context(), key); existsErr != nil || exists && h.Storage.Delete(r.Context(), key) != nil {
			middleware.WriteError(w, r, http.StatusConflict, "ARTIFACT_RETRY_REJECTED", "failed artifact bytes could not be cleaned safely")
			return
		}
		if tag, resetErr := h.Pool.Exec(r.Context(), `UPDATE artifacts SET status='PENDING',verified_at=NULL WHERE organization_id=$1 AND id=$2 AND status='FAILED'`, org, artifactID); resetErr != nil || tag.RowsAffected() != 1 {
			middleware.WriteError(w, r, http.StatusConflict, "ARTIFACT_RETRY_REJECTED", "artifact retry could not be prepared")
			return
		}
	}
...
func (h AgentArtifacts) Download(w http.ResponseWriter, r *http.Request) {
	...
	written, copyErr := io.CopyN(w, source, size)
	event, outcome := "artifact.transfer_completed", "success"
...
func (h AgentArtifacts) Upload(w http.ResponseWriter, r *http.Request) {
	...
	if copyErr != nil || written != size || written > h.MaxUploadBytes || !strings.EqualFold(actual, checksum) {
		...
		if written > h.MaxUploadBytes {
			reason, code, status = "size", "ARTIFACT_SIZE_LIMIT_EXCEEDED", http.StatusRequestEntityTooLarge
		}
...
func artifactStorageKey(organizationID, jobID, artifactID string) string {
	return "organizations/" + organizationID + "/jobs/" + jobID + "/artifacts/" + artifactID
}
func allowedArtifactContentType(value string) bool {
	return map[string]bool{"application/octet-stream": true, "application/json": true, "text/plain": true, "application/vnd.tcpdump.pcap": true}[value]
}
