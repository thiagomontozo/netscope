...
export interface AgentDetail {id:string;name:string;hostname:string;os:string;arch:string;version:string;protocol_version:string;contract_version?:string;capability_schema_version?:string;compatibility_status:'COMPATIBLE'|'UPGRADE_RECOMMENDED'|'INCOMPATIBLE'|'UNKNOWN';status:string;last_seen_at:string|null;registered_at:string;capabilities:JsonValue;capabilities_manifest:JsonValue;network_zone:string|null;identity_fingerprint:string;available_slots:number;running_jobs:number;certificate_serial?:string|null;certificate_expires_at?:string|null;certificate_rotation_status?:'IDLE'|'REQUESTED'|'ISSUED'|'GRACE_PERIOD'|'FAILED';signing_key_id?:string|null}
